...
Basket1Pin = 33 # GPIO 23 (WiringPi pin num 23) header pin 13

PowerPin   = 40 # GPIO 28 (WiringPi pin num 29) header pin 38

baskets = { '1': { 'pin': Basket1Pin, 'score': 0, 'display_offset': 1 } }
...
def setup():
	...
	max7219_send(SetDecodeMode, 0x07)
	max7219_send(SetDisplayTest, 0)
	max7219_send(SetIntensity, 1)
	max7219_send(SetScanLimit, 2)
	max7219_send(SetShutdown, 1)

	for basket_id, basket_info in baskets.items():
		GPIO.setup(basket_info['pin'], GPIO.IN)
		GPIO.add_event_detect(basket_info['pin'], GPIO.RISING, callback=made_basket, bouncetime = debounce_wait)
...
def made_basket(pin=None):
	...
	# GPIO.RISING detects both rising and falling edges.
	# This is a long-running bug:
	# https://www.raspberrypi.org/forums/viewtopic.php?f=32&t=59457
	# Just return if it's the falling edge.
